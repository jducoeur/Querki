...
lazy val scalaV = "2.11.1"
lazy val appV = "0.11.9.1"

lazy val sharedSrcDir = "querki-shared"

lazy val querkiServer = (project in file("querki-server")).settings(
  scalaVersion := scalaV,
  version := appV,
...
lazy val querkiClient = (project in file("querki-client")).settings(
  scalaVersion := scalaV,
  version := appV,
  ...
  libraryDependencies ++= sharedDependencies.value ++ Seq(
    ...
    // I don't think scala-parser-combinators has been published for Scala.js 0.6.x yet
    "org.scalajs" %% "scala-parser-combinators_sjs0.5" % "1.0.2",
