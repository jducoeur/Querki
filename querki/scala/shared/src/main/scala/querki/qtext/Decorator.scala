...
trait Decorator {
  ...
  def javascriptNeutralized(url: String): String = {
    ...
    if (
      !(urlLower.startsWith("http:") || urlLower.startsWith("https:") || urlLower.startsWith(
        "mailto:"
      )) && urlLower.matches("^[a-z0-9+\\.\\-]*:.*")
    )
      "./" + url
    else
      url
  ...
  def decorateLink(
    ...
    val isExternal = (nurlLower.startsWith("http:") || nurlLower.startsWith("https:") || nurlLower.startsWith(
      "mailto:"
    ) || nurlLower.startsWith("./") || nurlLower.startsWith("/"))
    val extAttrs =
      if (isExternal)
  ...
  def decorateClassSpan(
    className: String,
    text: String
  ): String = "<span class=\"" + className + "\">" + text + "</span>"
}
