...
//    def xmlAttrVal:Parser[String] = '"' ~> ((not('"') ~> aChar)*) <~ '"' ^^ {'"' + _.mkString + '"'}
  def xmlAttrVal: Parser[String] = oneOf(Set('"', '\'')) >> { delim =>
    ((not(delim) ~> aChar) *) ~ delim ^^ {
      case v ~ delim => {
  ...
  def xmlStartOrEmptyTag: Parser[String] =
    '<' ~> xmlName ~ ((xmlAttr | anchorTargetHack | selfTargetHack) *) ~ ows ~ (">" | "/>") ^^ {
      case name ~ attrs ~ w ~ e => {
        val allAttrs = {
          if (name.toLowerCase() == "a") {
            attrs.find(_._1 == "href") match {
              case Some(pair) => {
                ...
                val isExternal = (nurlLower.startsWith("http:") || nurlLower.startsWith(
                  "https:"
