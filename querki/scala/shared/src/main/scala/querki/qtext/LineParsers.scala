 ...
 */
sealed abstract class MarkdownLine(
  val prefix: String,
  val payload: String
) {
 ...
 */
case class LinkDefinitionStart(
  id: String,
  url: String
) {
 ...
 */
case class ParseFlag(
  name: String,
  v: String
) extends MarkdownLine(name, v)
...
case class MarkdownLineReader private (
  ...
  def handleFlagsRec(
    curLines: Seq[MarkdownLine],
    curLineCount: Int,
    curFlags: Map[String, Int]
  ): (Seq[MarkdownLine], Int, Map[String, Int]) = {
    if (curLines.isEmpty)
  ...
  def this(
    ls: Seq[MarkdownLine],
    lu: Map[String, LinkDefinition]
  ) = this(ls, lu, 1, Map.empty)
...
trait LineParsers extends InlineParsers {
  ...
  def pureXmlLine: Parser[String] = opt(ws) ~ rep1sep(xmlTag | xmlEndTag, opt(ws)) ^^ { case ws ~ xml =>
    ws.getOrElse("") + xml.mkString
  }
   ...
   * and any whitespace.
   */
  val dItemStartLine: Parser[DItemStartLine] =
    (""" {0,3}: """.r) ~ ("""(?:(?! :).)*""".r) ~ (""" : [\t\v ]*""".r) ~ rest ^^ {
   ...
   */
  val classDivStart: Parser[ClassDivStartLine] =
    """ {0,3}\{\{ *""".r ~ rep1sep("""[\w\-]+""".r, " +".r) ~ """ *: *""".r ^^ {
