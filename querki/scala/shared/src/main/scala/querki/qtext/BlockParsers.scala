...
trait BlockParsers extends Parsers {
  ...
  class ListItem(
    ...
    lookup: Map[String, LinkDefinition],
    val deco: Decorator = BlockParsers.this.deco
  ) extends LineParsers {
  ...
  def setExtHeader: Parser[Header] =
    not(emptyLine) ~> anyLine ~ line(classOf[SetExtHeaderLine]) ~ lookup ^^ { case l ~ setext ~ lu =>
   ...
   */
  def paragraph: Parser[Paragraph] = lookup ~ flags ~ (line(classOf[OtherLine]) +) ~ (emptyLine *) ^^ {
    case lu ~ lf ~ ls ~ empties => new Paragraph(ls, lu, lf, empties)
  }
   ...
   */
  def dItem: Parser[ListItem] =
    lookup ~ line(classOf[DItemStartLine]) ~ itemLines ~ (itemContinuation *) ~ optEmptyLines ^^ {
      case lu ~ s ~ ls ~ cs ~ e => new ListItem(s :: ls ++ cs.flatten ++ e, lu)
    }
  ...
  def apply[T](
    p: Parser[T],
    list: List[MarkdownLine]
  ): T = apply(p, new MarkdownLineReader(list))
