...
object HoconParse {
  ...
  def wOptP[Ctx: P]: P[Unit] = P(CharsWhile(_.isWhitespace, 0))

  // The separator between key-value pairs. Note that commas are optional in HOCON
  def kvSepP[Ctx: P]: P[Unit] = P(wOptP ~ ("," ~ wOptP).?)

  // A single "name", following JS syntax
  def nameFirstP[Ctx: P]: P[String] = P((CharPred(_.isLetter) | "_" | "$").!)
  def nameRestP[Ctx: P]: P[String] = P((CharsWhile(c => c.isLetterOrDigit | c == '_' | c == '$')).!)
  // TODO: if the quotes are present, they must be balanced:
  def nameP[Ctx: P]: P[String] = P("\"".? ~ (nameFirstP ~ nameRestP).! ~ "\"".?)

  // A path to a final element. This may be one or more levels deep:
  def pathP[Ctx: P]: P[Seq[String]] = P(nameP.rep(min = 1, sep = "." ~/ Pass))

  // Triple-quoted strings can contain anything, including newlines and ordinary quotes:
  def tripleQuotedP[Ctx: P]: P[HCValue] = P("\"\"\"" ~ (!"\"\"\"" ~ AnyChar).rep.! ~ "\"\"\"").map(SimpleValue(_))

  // Conventional quoted strings.
  // TODO: interpret \r, \n, etc, correctly.
  // TODO: reject newlines and other control chars inside single-quoted strings.
  def quotedP[Ctx: P]: P[HCValue] = P("\"" ~ (!"\"" ~ AnyChar).rep.! ~ "\"").map(SimpleValue(_))

  // A sub-object
  def objP[Ctx: P]: P[HCValue] = P("{" ~ objectGutsP ~ "}")

  def vP[Ctx: P]: P[HCValue] = P(tripleQuotedP | quotedP | objP)
  ...
  def kv[Ctx: P]: P[ObjectValue] = P(pathP ~ wOptP ~ (":" | "=") ~ wOptP ~ vP).map { case (path, v) =>
  ...
  def objectGutsP[Ctx: P]: P[ObjectValue] = P(wOptP ~ kv.rep(sep = kvSepP) ~ wOptP).map { kvs =>
  ...
  def topP[Ctx: P]: P[ObjectValue] = P("{".? ~ wOptP ~ objectGutsP ~ wOptP ~ "}".?)
