...
package object manifest {
  ...
  implicit def builder2MarcoPoloOptions(builder:MarcoPoloOptionBuilder) = builder._result

  implicit def jq2Manifest(jq:JQuery):ManifestFacade = jq.asInstanceOf[ManifestFacade]
  implicit def builder2ManifestOptions(builder:ManifestOptionBuilder) = builder._result
}
