...
package object manifest {
  ...
  implicit def manifest2Commands(manifest: ManifestFacade): ManifestCommands = new ManifestCommands(manifest)
  implicit def jq2ManifestCommands(jq: JQuery): ManifestCommands = new ManifestCommands(jq.asInstanceOf[ManifestFacade])
}
