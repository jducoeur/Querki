...
class PrintEcot(e: Ecology) extends ClientEcot(e) with Print {
  ...
  def printPrintView(
    ...
    val printFrame: Gadget[dom.HTMLIFrameElement] =
      iframe(cls := "_printFrame", display := "none", visibility := "hidden")
    ...
    $("link").filter({ e: dom.Element =>
      $(e).attr("rel").map(_ == "stylesheet").getOrElse(false)
    }).foreach({ e: dom.Element =>
      val newLink = $(e).clone()
      outstandingLinks += 1
