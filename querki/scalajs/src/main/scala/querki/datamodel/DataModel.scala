...
class DataModelEcot(e: Ecology) extends ClientEcot(e) with DataModel with querki.util.ScalatagUtils with PageImplicits {
  ...
  def showSpaceAfterDelete(
    thing: ThingInfo,
    recoverMsg: Option[String] = None
  ) = {
    Pages.showSpacePage(DataAccess.space.get).flashing(
      false,
      s"${thing.displayName} deleted. ",
      b(a(
        href := Pages.undeleteFactory.pageUrl("thingId" -> thing.oid.underlying, "thingName" -> thing.displayName),
        "Click here to undelete it."
      )),
        ...
        if (usage.nModels > 0 || usage.nInstances > 0) {
          val deleteDialog: Dialog =
            new Dialog(
              "Confirm Delete",
              p(b(
                s"""Property ${thing.displayName} is currently being used by ${usage.nModels} Models and 
              ...
              (
                ...
                Seq(s"Remove all values, then delete ${thing.displayName}", id := "_removeAllConfirm"),
                { dialog =>
          ...
          if (nInstances == 0)
            ...
            val deleteDialog: Dialog =
              new Dialog(
                ...
                (
                  ButtonGadget.Warning,
                  Seq("Delete", id := "_confirmDelete"),
                  { dialog =>
                    doDelete(
                      thing,
                      Some(dialog),
                      Some("You can find the ophaned Instances by going to Actions -> Advanced...")
                    )
                  }
                ),
                (ButtonGadget.Normal, Seq("Cancel", id := "_cancelDelete"), { dialog => dialog.done() })
  ...
  private def modelSelectionForm(
    formTitle: String,
    prompt: String,
    selectButton: String,
    onSelect: TID => Unit,
    ...
    for {
      ...
      val modelOptions = Var({
        val modelOpts =
          typeInfo.models.filter(_.isInstantiatable).sortBy(_.displayName).map(model =>
            option(value := model, model.displayName)
          )
      ...
      val modelDialog: Dialog =
        new Dialog(
          ...
          (
            ...
            Seq(selectButton, id := "_modelSelected"),
            { dialog =>
