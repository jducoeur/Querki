...
class PublicationEcot(e: Ecology) extends ClientEcot(e) with Publication {
  ...
  lazy val editPublicationFactory =
    Pages.registerThingPageFactory("_editPublish", { (params) => new EditPublicationPage(params) }, "modelId")
  ...
  def discardChanges(
    ...
    val confirmDialog = new Dialog(
      "Confirm Discard",
      p(b(
        s"""Are you sure you want to discard changes to ${thing.displayName}? They can not be recovered afterwards.""".stripMargin
      )),
      (
        ButtonGadget.Warning,
        Seq(s"Discard changes to ${thing.displayName}", id := "_discardConfirm"),
        { dialog =>
