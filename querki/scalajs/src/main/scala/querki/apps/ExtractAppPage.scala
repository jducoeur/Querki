    ...
    for {
      ...
      guts =
        div(
          h1("Extract an App"),
          QText(
            """This page lets you create an **App** based on this Space. You do this by "extracting" the structure
          ...
          br(),
          p(b("What should the new App be named?")),
          ...
          br(),
          p(b("Briefly (one line) describe the new App")),
          summaryInput <= new RxText(
            id := "_appSummary",
            cls := "form-control",
            value := getPropVal(std.conventions.summaryProp, spaceProps)
          ),
          br(),
          p(b("Give a bit more description of the new App")),
          detailsInput <= new RxTextArea(
            id := "_appDetails",
            cls := "_largeTextEdit form-control",
            getPropVal(std.conventions.detailsProp, spaceProps)
          ),
          br(),
          p(b("In the list below, select the Models and Instances to lift into the new App.")),
          ...
          new ButtonGadget(ButtonGadget.Warning, "Extract App from this Space", disabled := notReady)({ () =>
            ...
            Client[AppsFunctions].extractApp(
              ...
              summaryInput.get.text.now,
              detailsInput.get.text.now
            ).call().foreach { spaceInfo =>
...
class ExtractTree(
  models: Seq[ExtractableModelInfo],
  pages: Seq[ThingInfo]
)(implicit
  val ecology: Ecology
) extends Gadget[dom.html.Div]
     with EcologyMember {
  ...
  val spaceNode =
    JsTreeNode
      ...
      .state(NodeState.Selected): JsTreeNode
            ...
            Client[ThingFunctions].getChildren(tid, false, true).call().foreach { result =>
              val instanceNodes = result.map { instanceInfo =>
                ...
                JsTreeNode
                  ...
                  .state(instanceState: _*): JsTreeNode
              }
