...
class DescriptionDiv(
  ...
  selector: Rx[Option[(RxThingSelector, TID)]]
)(implicit
  val ecology: Ecology,
  ctx: Ctx.Owner
        ...
        val fut =
          ...
          div(
            ...
            propMap.get(oid).map { propInfo =>
              val typeInfoOpt: Option[BasicThingInfo] =
                typeMap.get(propInfo.typeId).orElse(modelMap.get(propInfo.typeId))
              i(
                " (",
                thingLink(collMap(propInfo.collId)),
