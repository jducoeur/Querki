...
class EditingEcot(e: Ecology) extends ClientEcot(e) with Editing {
  ...
  lazy val editInstancesFactory =
    Pages.registerThingPageFactory("_editInstances", { (params) => new EditInstancesPage(params) }, "modelId")

  lazy val modelDesignerFactory =
    Pages.registerThingPageFactory("_modelDesigner", { (params) => new ModelDesignerPage(params) }, "modelId")

  lazy val advancedEditorFactory =
    Pages.registerThingPageFactory("_advancedEditor", { (params) => new ModelDesignerPage(params) }, "thingId")

  lazy val editSpaceInfoFactory =
    Pages.registerStandardFactory("_editSpaceInfo", { (params) => new EditSpaceInfoPage(params) })
  ...
  def getSomePropertyEditors(
    ...
    Future.sequence(futs).map(infos => infos.map { info => (info.propInfo.oid, new RawDiv(info.editor)) }).map(Map(
      _: _*
    ))
  }
}
