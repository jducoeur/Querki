...
class EditSpaceInfoPage(params: ParamMap)(implicit val ecology: Ecology)
  extends Page("editSpaceInfo")
     with querki.security.LevelMap {
  ...
  def pageContent = {
    for {
      ...
      (propEditors, (allPerms, spacePerms)) <-
        Editing.getSomePropertyEditors(
          spaceInfo.oid,
      ...
      guts =
        div(
          ...
          p(
            "These are the most common properties to edit for your Space. When you are finished, press the Done button."
          ),
          ...
          p(
            "Or you can ",
            a(href := Pages.securityFactory.pageUrl(spaceInfo), "customize the permissions however you prefer.")
          ),
          br(),
          p(SpaceLinkButton(ButtonGadget.Primary, id := "_spaceInfoDoneButton", "Done"))
        )
