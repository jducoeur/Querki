...
class RxSelect(
  options: Rx[Seq[Frag]],
  emptyText: Option[String],
  mods: Modifier*
)(implicit
  val ecology: Ecology,
  ctx: Ctx.Owner
) extends Gadget[dom.HTMLSelectElement]
     with RxThingSelector {
...
object RxSelect {
  ...
  def apply(
    ...
    ecology: Ecology,
    ctx: Ctx.Owner
  ) = new RxSelect(options, Some(emptyText), mods)
  ...
  def apply(
    ...
    ecology: Ecology,
    ctx: Ctx.Owner
  ) = new RxSelect(options, None, mods)
  ...
  def apply(
    ...
    ecology: Ecology,
    ctx: Ctx.Owner
  ) = new RxSelect(Var(Seq.empty), None, mods)
}
