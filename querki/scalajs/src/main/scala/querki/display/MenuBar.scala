...
class MenuBar(
  ...
  e: Ecology,
  ctx: Ctx.Owner
) extends HookedGadget[dom.HTMLDivElement](e)
     with QuerkiUIUtils
  ...
  def alwaysLinks: Seq[Navigable] = {
    Seq(
      NavLink(
        "Refresh",
        id = "_refreshMenuItem",
        onClick = Some({ () => PageManager.reload() }),
  ...
  def thingLinks: Option[Seq[Navigable]] = {
    thingOpt.map { thing =>
      ...
      Seq(
        ...
        NavLink(
          "View Source",
          Pages.viewFactory.pageUrl(thing),
          requiresExplore = true,
          complexity = Standard,
          allowedDuringHistory = true
        ),
        NavLink(
          s"Security for ${thing.displayName}",
          Pages.securityFactory.pageUrl(thing),
          requiresExplore = true,
          enabled = canShare,
          id = "_securityItem",
          complexity = Advanced
        ...
        NavLink(
          "Explore...",
          Pages.exploreFactory.pageUrl(thing),
          requiresExplore = true,
          complexity = Standard,
          allowedDuringHistory = true
        ),
        ...
        (if (thing.isModel)
           Seq(NavLink(
             "Create a " + thing.displayName,
             Pages.createAndEditFactory.pageUrl(thing),
             requiresExplore = true
           ))
         else
           Seq.empty)
  ...
  def loginSection = {
    UserAccess.user match {
      case Some(user) => {
        if (user.actualUser) {
          NavSection(
            truncateName(user.mainIdentity.name),
            Seq(
              NavLink("Your Account", Pages.accountFactory.pageUrl()),
              NavLink("Your Spaces", url = "/", allowedDuringHistory = true),
              NavLink(
                s"${SkillLevel.current.name} (change)",
                onClick = Some({ () => SkillLevel.changeSkillLevel() }),
                id = "_skillLevelButton"
              ),
  ...
  def doRender() =
    div(
      cls := "container",
      div(
        cls := "navbar navbar-default navbar-fixed-top _noPrint",
        role := "navigation",
        div(
          cls := "container",
          div(
          ...
          div(
            cls := "querki-navbar-collapse navbar-collapse collapse",
            ul(
              cls := "nav navbar-nav",
            ...
            if (DataAccess.space.isDefined && hasExplore) {
              form(
                ...
                role := "search",
                tabindex := 1800,
                div(cls := "form-group", new SearchGadget())
              )
            },
            if (UserAccess.isActualUser) {
              ul(cls := "nav navbar-nav navbar-right", tabindex := 1700, li(new NotifierGadget))
            }
          )
        )
      )
