...
trait InputDependencies {
  def register(gadget: DependentInputGadget, onPath: String): Unit
  def unregister(gadget: DependentInputGadget, onPath: String): Unit
...
class InputDependenciesHandler extends InputDependencies {
  def register(gadget: DependentInputGadget, onPath: String) = {
    val ds: Set[DependentInputGadget] = dependencies.get(onPath).getOrElse(Set.empty)
    dependencies += (onPath -> (ds + gadget))
  }
  
  def unregister(gadget: DependentInputGadget, onPath: String) = {
    val ds: Set[DependentInputGadget] = dependencies.get(onPath).getOrElse(Set.empty)
    dependencies += (onPath -> (ds - gadget))    
  }
