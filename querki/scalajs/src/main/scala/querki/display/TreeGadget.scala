...
class TreeGadget(implicit e: Ecology) extends HookedGadget[dom.html.Div](e) {
  ...
  def hook() = {
    $(elem).jsTree(JsTreeOptions.core(JsTreeCore.themes(JsTreeTheme.dots(false))))
  }
}
...
class QLTree(implicit e: Ecology) extends HookedGadget[dom.html.Div](e) with QuerkiUIUtils {
  ...
  def hook() = {
    ...
    }).themes(JsTreeTheme.dots(false).responsive(true))))
      .on(
        "after_open.jstree",
        { (opened: dom.Element, evt: JQueryEventObject) =>
          hookClick()
        }
      )
    $(elem).remove()
