...
trait QuerkiUIUtils extends ScalatagUtils {
  ...
  def querkiButton(
    show: Modifier,
    addlCls: Seq[String] = Seq.empty
  ) = plainQuerkiButton(show, "btn-primary" +: addlCls)

  def plainQuerkiButton(
    show: Modifier,
    addlCls: Seq[String] = Seq.empty
  ) =
    a(classes(Seq("btn", "btn-default", "btn-sm", "_noPrint", "querki-icon-button") ++ addlCls), show)
  ...
  def iconButton(
    iconName: String,
    addlCls: Seq[String] = Seq.empty
  ) = plainQuerkiButton(icon(iconName), addlCls)

  def faIconButton(
    iconName: String,
    addlCls: Seq[String] = Seq.empty
  ) = plainQuerkiButton(faIcon(iconName), addlCls)
