 ...
 */
class PagePaginator(nElements:Int, urlConstr:ParamMap => URL, params:ParamMap)(implicit val ecology:Ecology) extends Gadget[dom.HTMLDivElement] with EcologyMember {
    ...
    if (totalPages > 1) {
      ...
      div(
        ul(cls:="pagination pagination-centered",
          ...
          range.map { pageNum =>
            li(cls:={ if (pageNum == currentPage) "active" else "" }, 
              a(href:=pageUrl(pageNum), pageNum.toString))
          },
