...
private[security] class PermCheckboxes(
  allPerms: Seq[PermInfo],
  thingPropsOpt: Option[Seq[PropValInfo]],
  std: StandardThings
)(implicit
  e: Ecology,
  ctx: Ctx.Owner
) extends InputGadget[html.Div](e)
     with NoAutoSave
     with ForProp {
  ...
  def doRender() =
    div(cls := "container", p(b("This Role gives its members these permissions throughout this Space")), checkboxes)
}
