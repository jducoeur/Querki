...
class CustomRoleList(
  ...
  ctx: Ctx.Owner
    ...
    div(
      ...
      p(
        """Click on a Role to edit it, or to create a Shared Link that will give that Role to anyone who clicks on it.""".stripMargin
      )
    )
  ) {
...
class CustomRolesTab(
  ...
  val ecology: Ecology,
  ctx: Ctx.Owner
) extends TabGadget(SharingPage.Tab.CustomRoles.entryName, "custom", "Roles")
     with EcologyMember {
