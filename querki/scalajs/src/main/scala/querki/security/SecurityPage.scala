...
trait LevelMap { page: Page =>
  ...
  def currentPermLevel(
    permInfo: PermInfo,
    thingPerm: Option[ThingPerm],
    isSpace: Boolean
  ): SecurityLevel = {
 ...
 * @author jducoeur
 */
class SecurityPage(params: ParamMap)(implicit val ecology: Ecology) extends Page("security") with LevelMap { self =>
  ...
  class ShowPerms(
    t: ThingInfo,
    kindPerms: Seq[PermInfo],
    thingPerms: Seq[ThingPerm],
    isSpace: Boolean
  )(implicit
    val ecology: Ecology
  ...
  def pageContent = for {
    ...
    guts =
      div(
        h2(
          s"Security ",
          new ButtonGadget(ButtonGadget.Normal, id := "_doneButton", "Done")({ () =>
        ...
        ul(
          ...
          li(
            role := "presentation",
            cls := "active",
            a(href := "#secThis", role := "tab", attr("data-toggle") := "tab", thing.displayName)
          ),
          if (hasInstancePerms)
        ...
        div(
          ...
          if (hasInstancePerms && perms.instancePermThing.isDefined)
            section(
              ...
              new ShowPerms(
                perms.instancePermThing.get,
                filterPermsFor(allPerms, appliesToInstances),
                perms.instancePerms,
                isSpace
              )
            )
