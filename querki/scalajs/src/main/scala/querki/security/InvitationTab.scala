...
class InvitationTab(
  ...
  val ecology: Ecology,
  ctx: Ctx.Owner
) extends TabGadget(SharingPage.Tab.Invite.entryName, "sendInvitations", "Invites")
     with EcologyMember
     with PageImplicits {
  ...
  class InviteeInput extends InputGadget[html.Input](ecology) {
    ...
    def hook() = {
      ...
      $(elem).manifest(ManifestOptions.marcoPolo(false).
      // 188 is the *keycode* for comma:
      separator(Seq[Int](13, ',', 188).toJSArray).valuesName("invitees"))
    }
  ...
  class CollaboratorInput extends InputGadget[html.Input](ecology) {
    ...
    def hook() = {
      ...
      $(elem).manifest(ManifestOptions.marcoPolo(
        MarcoPoloOptions.url("_getCollaborators").minChars(3).required(true).formatData({ (data: js.Array[js.Object]) =>
          data
        }).formatItem({ data: js.Dynamic => data.display }).formatNoResults({ q: String =>
          s"No collaborator named $q found.".asInstanceOf[js.Any]
        })
      ).
      // 188 is the *keycode* for comma:
      separator(Seq[Int](13, ',', 188).toJSArray).required(true).valuesName("collaborators").formatDisplay({
        data: js.Any => data.asInstanceOf[ManifestItem].display
      }: Function1[js.Any, js.Any]).formatValue({ data: js.Object => data.asInstanceOf[ManifestItem].id }))
    }
  ...
  def tabContent =
    for {
      ...
      inviteEditInfo <-
        Client[EditFunctions].getOnePropertyEditor(DataAccess.space.get.oid, std.security.inviteTextProp).call()
      awaitingValidation = (DataAccess.request.userLevel == PendingUser)
      guts =
        div(
          h3("Send Invitations to Join this Space"),
          p(
            """Use this form to invite people to become Members of this Space. The Invitation Message may contain the usual Querki Text formatting,
              |including [[]]-style expressions; however, links may not yet work quite the way you expect.""".stripMargin
          ),
          p(
            """Specify invitees by email address. Note that your current invitations are listed below. While it is acceptable to retry once or
              |twice, doing so excessively is frowned upon, as is sending unwelcome invitations.""".stripMargin,
            br(),
          ...
          p(
            """Invitations will have your email address included as the Reply-To, so if the invitees write back, it will go directly to you.
          ...
          div(
            ...
            div(
              ...
              new RolesDisplay(
                securityInfo.defaultRoles,
                DataAccess.space.get.oid,
                roleMap,
                customMap,
                customDisplay,
                std
