...
import querki.display.{ButtonGadget, Dialog, TabGadget}
  ...
  lazy val deleteDialog: Dialog =
    new Dialog(
      ...
      (
        ...
        Seq("Remove Members", id := "_confirmRemoveButton"),
        { dialog =>
          dialog.done()
    ...
    for {
      ...
      guts =
        div(
          h3("Members"),
          p(
            "The following people are members of this Space. Click on a member's Role in order to change it. Click on a row to select that member."
          ),
