 ...
 * @author jducoeur
 */
class SignUpPage[T](includeSignin: Boolean)(onReady: Option[UserInfo => T])(implicit val ecology: Ecology)
  extends Page("signup") {
  ...
  def showTOSPanel(tosInfo: TOSInfo) = {
    ...
    div(
      div(
        ...
        div(cls := "panel-body", div(cls := "well _signupTosBody", new QText(text)))
      ),
    ...
    guts = div(
      if (includeSignin) {
        div(
          ...
          // shouldn't be:
          p(
            b(DataAccess.space.get.displayName),
            s" requires that you log in (if you have an account already), or sign up below."
          ),
          form(
            showInput(
              logic.handleInput,
              ...
              "text",
              "joe@example.com",
              "Must be your handle or email address",
              loginHandleOkay,
              None
            ),
            showInput(
      ...
      form(
        ...
        showInput(
          handleInput,
          Some(InputUtils.nameFilter(false, false)),
          "Choose a Querki handle",
          "handleInput",
          "text",
          "Handle",
        ...
        showTOSPanel(tos),
        signupButton <= new RunButton(
          ButtonGadget.Primary,
          "Sign Up",
          "Signing up...",
          id := "signupButton",
          disabled := Rx { !signupEnabled() }
        )({ _ => signup(tos) })
      )
    )
