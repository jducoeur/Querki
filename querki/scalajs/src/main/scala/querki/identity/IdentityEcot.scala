...
class IdentityEcot(e: Ecology) extends ClientEcot(e) with Identity {
  ...
  lazy val handleInviteFactory =
    Pages.registerStandardFactory("_handleInvite", { (params) => new HandleInvitePage(params) })
  ...
  lazy val validateSignupFactory =
    Pages.registerStandardFactory("_validateSignup", { (params) => new ValidateSignupPage(params) })
