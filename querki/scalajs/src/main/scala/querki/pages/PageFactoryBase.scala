...
class ThingPageFactoryBase(
  registeredName: String,
  const: ParamMap => Page,
  paramName: String
)(implicit
  e: Ecology
) extends PageFactoryBase(registeredName, const)
     with ThingPageFactory {

  def pageUrl(
    thing: BasicThingInfo,
    addlParams: (String, String)*
  ): URL = pageUrl((addlParams :+ (paramName -> thing.urlName.underlying)): _*)
