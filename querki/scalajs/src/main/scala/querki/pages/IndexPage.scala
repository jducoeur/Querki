...
class IndexPage(params: ParamMap)(implicit val ecology: Ecology) extends Page("index") {
  ...
  def spaceSection(
    title: String,
    section: Seq[SpaceInfo]
  ) = {
    div(
      cls := "col-md-6",
      h3(title),
      ul(
        for { space <- section } yield showSpace(space)
      )
    )
  }
  ...
  def awaitingValidationSection =
    div(
      ...
      p(
        """An email has been sent to your address, with a validation link. Please find that email and click on the
