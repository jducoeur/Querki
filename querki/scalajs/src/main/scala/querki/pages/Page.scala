...
case class PageContents(
  title: String,
  content: TypedTag[dom.HTMLDivElement]
) {
...
//    ctx.kill()
  ...
  def doRender() = {
    ...
    val outerPage = div(
      cls := "guts container-fluid",
      div(
        cls := "row",
        div(
          cls := "querki-content col-md-12",
          ...
          DataAccess.space match {
            case Some(space) =>
              div(
                cls := "_smallSubtitle _spaceLink _noPrint",
