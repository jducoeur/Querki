    ...
    pageFut.onFailure {
      ...
      case UnknownThingException(thingId) =>
        PageManager.showRoot().map { page =>
          page.flash(true, s"There is no Thing with the ID $thingId")
        }
...
class StandardThingHeader(
  ...
  lazy val shareButton =
    div(
      cls := "btn-group querki-icon-button",
      button(
        tpe := "button",
      ...
      ul(
        ...
        if (isSpace && canShare) {
          MSeq(
            li(a(
              href := Pages.sharingFactory.pageUrl("tab" -> querki.security.SharingPage.Tab.CustomRoles.entryName),
              "Get Shareable Link..."
            )),
  ...
  def doRender =
    div(
      ...
      h1(
        ...
        Gadget(
          faIconButton("refresh")(title := "Refresh this page"),
          { e =>
      ...
      if (hasExplore) {
        modelOpt match {
          case Some(model) if (!thing.isTag) => {
            p(
              cls := "_smallSubtitle _noPrint",
              "(OID: ",
              a(href := page.thingUrl(thing.oid), id := "_thingOID", thing.oid.underlying),
              thing.linkName.map { linkName =>
                MSeq(", Link Name: ", a(href := page.thingUrl(thing.urlName), linkName))
              },
              ", Model: ",
              a(href := page.thingUrl(model), model.displayName),
              ")"
            )
          }
          case Some(model) if (thing.isTag) => {
            p(cls := "_smallSubtitle _noPrint", "(Tag based on ", thingLink(model), ")")
          }
          case None => {}
        }
      }
