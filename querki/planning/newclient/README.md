...
| [reference-philosophy-of-testing.md](../reference-philosophy-of-testing.md) | Condensed summary of the owner's 8-part "A Philosophy of Testing" Medium series — the authoritative intent behind the testing approach (scenario tests, nuanced 100%, determinism, code-for-testing). **Lives in `planning/` (parent dir), not `newclient/`** — it's a general long-term reference for the whole app, not specific to the rewrite. |
