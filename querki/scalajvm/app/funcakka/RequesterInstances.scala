...
trait RequestMInstances extends RequestMInstances1 {
  ...
  implicit val catsStdInstancesForRequestM: MonadError[RequestM, Throwable]
    with CoflatMap[RequestM]
    with Monad[RequestM] =
    new RequestMCoflatMap with MonadError[RequestM, Throwable] with Monad[RequestM] {
      ...
      def handleErrorWith[A](fea: RequestM[A])(f: Throwable => RequestM[A]): RequestM[A] = fea.recoverWith { case t =>
        f(t)
      }
      ...
      override def recoverWith[A](fa: RequestM[A])(pf: PartialFunction[Throwable, RequestM[A]]): RequestM[A] =
        fa.recoverWith(pf)
