...
trait ThingEditor { self: EditorModule =>
   ...
   */
  def editorSpan(prop: Property[_, _])(implicit state: SpaceState): Int =
    prop.getPropOpt(EditWidthProp).flatMap(_.firstOpt).getOrElse(prop.pType.editorSpan(prop))
  ...
  private case class EditorPropLayout(prop: Property[_, _])(implicit state: SpaceState) extends LayoutElement {
    ...
    def displayNamePhrase: Option[String] =
      displayNameOpt.map(displayName =>
  ...
  private def propsToEditForThing(
    thing: PropertyBundle,
    state: SpaceState
  ): Iterable[Property[_, _]] = {
    implicit val s = state
    val result = for {
  ...
  val thingButtons =
    """{{_advancedEditButton:<i class="glyphicon glyphicon-edit btn-xs _withTooltip" title="Click to open the Advanced Editor"></i>}}
      |{{_deleteInstanceButton:<i class="glyphicon glyphicon-trash btn-xs _withTooltip" title="Click to delete this"></i>}}""".stripMargin

  private def editorLayoutForThing(
    thing: PropertyBundle,
    state: SpaceState
  ): QLText = {
    implicit val s = state
    thing.getPropOpt(InstanceEditViewProp).flatMap(_.v.firstTyped(LargeTextType)) match {
      ...
      case None => {
        ...
        val propsLayout = s"""[[""{{_instanceEditor:
                             ...
                             |}}"" ${if (thing.isThing)
          s"""-> _data(""thingId"", ""${thing.asInstanceOf[Thing].toThingId}"")"""
        else ""}]]
                             |""".stripMargin
        QLText(propsLayout)
      }
  ...
  def instanceEditorForThing(
    thing: PropertyBundle,
    thingContext: QLContext,
