...
class IdentityEcot(e: Ecology)
  extends QuerkiEcot(e)
     with IdentityAccess
     with querki.core.MethodDefs
     with UserCacheAccess {
  ...
  val identityResolver: ShardRegion.ExtractShardId = msg =>
    msg match {
  ...
  val userResolver: ShardRegion.ExtractShardId = msg =>
    msg match {
      case msg: UserCacheRequest => userShard(msg)
    }

  override def createActors(createActorCb: CreateActorFunc): Unit = {
    _ref = SystemManagement.createShardRegion(
      "IdentityCache",
      IdentityCache.actorProps(ecology),
      identityExtractor,
      identityResolver
    )
    _userRef =
      SystemManagement.createShardRegion("UserCache", UserCache.actorProps(ecology), userExtractor, userResolver)
  }

  override def persistentMessages = persist(39, (classOf[IdentityPersistence.UserRef] -> 100))
  ...
  def getIdentitiesInternal[T >: FullIdentity](
    ids: Seq[OID],
    notFound: (Map[OID, T], OID) => Map[OID, T]
  ): Future[Map[OID, T]] = {
  ...
  lazy val DisplayIdentity = ThingState(
    DisplayIdentityOID,
    systemOID,
    RootOID,
    toProps(
  ...
  lazy val IdentityType = new SystemType[PublicIdentity](
    IdentityTypeOID,
    toProps(
  ...
  lazy val ResolveIdentity = new InternalMethod(
    ResolveIdentityOID,
    toProps(
