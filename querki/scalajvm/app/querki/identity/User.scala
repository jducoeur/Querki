  ...
  def requireAdmin[T](f: => T): T = {
    if (isAdmin)
      ...
      throw new InternalException(
        "Illegal attempt to call a function that requires admin rights. Caller is " + toThingId
      )
  }
...
case class FullUser(
  ...
  level: UserLevel = UnknownUserLevel,
  tosVersion: Int = 0
) extends User
...
case class SimpleIdentity(
  id: OID,
  handle: String,
  name: String
) extends PublicIdentity
...
object Identity {
  ...
  object AnonymousIdentity
    extends Identity(
      AnonymousOID,
      EmailAddress("anonidentity@querki.net"),
      "",
      "Anonymous",
      "Anonymous",
