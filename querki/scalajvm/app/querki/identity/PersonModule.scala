 ...
 * directly.
 */
private[identity] case class CachedPeople(
  val ecology: Ecology,
  state: SpaceState
  ...
  lazy val (allPeopleById, allPeopleByIdentityId) =
    ((Map.empty[OID, Thing], Map.empty[IdentityId, Thing]) /: state.descendants(PersonOID, false, true, false)) {
      (maps, person) =>
        ...
        val newIdentityIdMap = getPersonIdentityRaw(person).map(identityId =>
          identityIdMap + (identityId -> person)
        ).getOrElse(identityIdMap)
 ...
 */
class PersonModule(e: Ecology)
  extends QuerkiEcot(e)
     with Person
     with querki.core.MethodDefs
     with Contributor[CacheUpdate, CacheUpdate] {
  ...
  def notify(
    evt: CacheUpdate,
    sender: Publisher[CacheUpdate, CacheUpdate]
  ): CacheUpdate = {
  ...
  lazy val InvitationStatusModel = ThingState(
    InvitationStatusOID,
    systemOID,
    RootOID,
    toProps(
  ...
  lazy val StatusInvited = ThingState(
    StatusInvitedOID,
    systemOID,
  ...
  lazy val StatusRequested = ThingState(
    StatusRequestedOID,
    systemOID,
    InvitationStatusModel,
    toProps(
  ...
  lazy val StatusMember = ThingState(
    StatusMemberOID,
    systemOID,
    InvitationStatusModel,
    toProps(
  ...
  lazy val StatusRejected = ThingState(
    StatusRejectedOID,
    systemOID,
    InvitationStatusModel,
    toProps(
  ...
  lazy val StatusRemoved = ThingState(
    StatusRemovedOID,
    systemOID,
    InvitationStatusModel,
    toProps(
  ...
  lazy val OwnedThings = ThingState(
    OwnedThingsOID,
    systemOID,
    RootOID,
    toProps(
  ...
  lazy val IdentityLink = new SystemProperty(
    IdentityLinkOID,
    LinkType,
    Optional,
    toProps(
  ...
  lazy val InviteText = new SystemProperty(
    InviteTextOID,
    LargeTextType,
    ExactlyOne,
    toProps(
  ...
  lazy val InvitationStatusProp = new SystemProperty(
    InvitationStatusPropOID,
    LinkType,
    Optional,
    toProps(
  ...
  lazy val IsSimpleGuestProp = new SystemProperty(
    IsSimpleGuestOID,
    YesNoType,
    Optional,
    toProps(
  ...
  lazy val meMethod = new InternalMethod(
    MeMethodOID,
    toProps(
      setName("_me"),
      Categories(IdentityTag),
   ...
   */
  lazy val PersonIdentityFunction = new InternalMethod(
    PersonIdentityFunctionOID,
    toProps(
  ...
  lazy val CreatorFunction = new InternalMethod(
    CreatorFunctionOID,
    toProps(
      setName("_creator"),
  ...
  def inviteMembers(
    rc: RequestContext,
    inviteeEmails: Seq[EmailAddress],
    ...
    val inviteeRoles: Seq[OID] = {
      val actual = originalState.getPropOpt(AccessControl.PersonRolesProp).map(_.rawList).getOrElse(Seq())
    ...
    withCache { cache =>
      for {
        ...
        (existing, newInvites) = inviteeMap.values.partition { identity =>
          cache.allPeopleByIdentityId.contains(identity.id)
        }
  ...
  object InviteLoginChecker extends Contributor[PlayRequestContext, PlayRequestContext] {
    ...
    def notify(
      ...
      sender: Publisher[PlayRequestContext, PlayRequestContext]
    ): PlayRequestContext = {
      val rcOpt =
        for {
          ...
          sessionUpdates = rc.sessionUpdates ++ rc.returnToHereUpdate ++ updates,
   ...
   * will get.
   */
  private def acceptOpenInvitationForRole(
    rc: RequestContext,
    role: Thing
  )(implicit
    state: SpaceState
        ...
        case Some(person) => {
          ...
          if (existingRoles.contains(roleId))
            ...
            val msg = ChangeProps(
              ...
              state.id,
              person.id,
              toProps(AccessControl.PersonRolesProp((existingRoles :+ roleId): _*))
            )
