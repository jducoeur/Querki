...
class InvitationNotifierEcot(e: Ecology) extends QuerkiEcot(e) with Notifier with EmailNotifier with NotifyInvitations {
  ...
  override def persistentMessages =
    persist(66, (classOf[DHUnsubSenderInvites] -> 100), (classOf[DHUnsubAllInvites] -> 101))
  ...
  def notifyInvitation(
    ...
    Person.withCache { cache =>
      ...
      invitees.foreach { invitee =>
        ...
        // an accepted Member:
        val person = cache.allPeopleByIdentityId.get(invitee.id).getOrElse(throw new Exception(
          s"Space ${state.id} doesn't contain a Person record for Identity $invitee!"
        ))
  ...
  case class InvitePayload(
    senderId: IdentityId,
    textQVOpt: Option[QValue],
    inviteUrl: String,
    senderName: String,
    spaceName: String,
    spaceOwner: OID
  ...
  def toEmail(
    ...
    val InvitePayload(senderId, textQVOpt, url, senderName, spaceName, spaceOwner) = parsePayload(note)
    val spaceId =
      spaceIdOpt.getOrElse(throw new Exception(s"Somehow got InvitationNotification with no spaceId: $note"))
    ...
    for {
      ...
      instructions =
        if (recipient.kind == IdentityKind.SimpleEmail)
          Wikitext(
            s"""Clicking on this link will let you set up a Querki login (for free) and begin using '$spaceName'.""".stripMargin
          )
        else
          Wikitext("")
      onlyYou = Wikitext("This link is intended for you personally; please do not share it with others.")
      joinButton = HtmlWikitext(
        s"""<div class="bottomlinkdiv"><a href="$url" class="btn btn-primary">Join Space '$spaceName'</a></div>""".stripMargin
      )
      fullBody =
        Wikitext("{{title:") + wikibreak + headline + wikibreak + Wikitext("}}") + wikibreak +
          ...
          onlyYou + wikibreak +
          joinButton
      unsubLink = Unsubscribe.generateUnsubLink(
        this,
        recipient.id,
        recipient.email,
        spaceId.toString,
  ...
  def getUnsubEvent(
    ...
    unsubId match {
      case UnsubSenderOID => {
        val sender =
          context.map(OID(_)).getOrElse(throw new Exception(s"Somehow got an UnsubSender request without the sender!"))
  ...
  lazy val InvitationSender = new SystemProperty(
    InvitationSenderOID,
    LinkType,
    ExactlyOne,
    toProps(
  ...
  lazy val InvitationText = new SystemProperty(
    InvitationTextOID,
    TextType,
    Optional,
    toProps(
  ...
  lazy val InvitationURL = new SystemProperty(
    InvitationURLOID,
    PlainTextType,
    ExactlyOne,
    toProps(
  ...
  lazy val InvitationSenderName = new SystemProperty(
    InvitationSenderNameOID,
    PlainTextType,
    ExactlyOne,
    toProps(
  ...
  lazy val InvitationSpaceName = new SystemProperty(
    InvitationSpaceNameOID,
    PlainTextType,
    ExactlyOne,
    toProps(
  ...
  lazy val InvitationSpaceOwner = new SystemProperty(
    InvitationSpaceOwnerOID,
    LinkType,
    ExactlyOne,
    toProps(
