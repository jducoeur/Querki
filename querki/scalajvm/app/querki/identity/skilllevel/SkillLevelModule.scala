...
class SkillLevelModule(e: Ecology) extends QuerkiEcot(e) with SkillLevel with Contributor[CacheUpdate, CacheUpdate] {
  ...
  private def calculatePropsBySkill(implicit state: SpaceState): Map[OID, Seq[Property[_, _]]] = {
    state.propList.toSeq.filterNot(_.ifSet(Basic.DeprecatedProp)).filterNot(_.ifSet(Core.InternalProp)).filterNot(
      _.ifSet(DataModel.IsFunctionProp)
    ).groupBy(apply(_))
  }
  ...
  def standardProps(implicit state: SpaceState): Seq[Property[_, _]] =
    propsForLevel(querki.identity.skilllevel.MOIDs.SkillLevelStandardOID)

  def advancedProps(implicit state: SpaceState): Seq[Property[_, _]] =
    propsForLevel(querki.identity.skilllevel.MOIDs.SkillLevelAdvancedOID)
  ...
  lazy val SkillLevelProp = new SystemProperty(
    SkillLevelPropOID,
    LinkType,
    ExactlyOne,
    toProps(
      ...
      Details(
        """Querki tries very hard to be simple, but at the same time includes a great deal of power.
  ...
  lazy val skillLevelModel = ThingState(
    SkillLevelOID,
    systemOID,
    RootOID,
    toProps(
  ...
  lazy val skillLevelBasic = ThingState(
    SkillLevelBasicOID,
    systemOID,
    SkillLevelOID,
    toProps(
      ...
      Categories(SkillLevelTag),
      Summary(
        """This level should be used for the core of Querki: the powerful but easy features that everyone uses."""
      )
    )
  )

  lazy val skillLevelStandard = ThingState(
    SkillLevelStandardOID,
    systemOID,
    SkillLevelOID,
    toProps(
      ...
      Categories(SkillLevelTag),
      Summary(
        """This level should be used for the bulk of Querki: the common features that are often used, but not quite core."""
      )
    )
  )

  lazy val skillLevelAdvanced = ThingState(
    SkillLevelAdvancedOID,
    systemOID,
    SkillLevelOID,
    toProps(
      ...
      Categories(SkillLevelTag),
      Summary(
        """This level should be used for the power-user features of Querki, which aren't quite as easy but provide the most flexibility."""
      )
    )
  )
