 ...
 */
case class ModeledPropertyBundle(
  modelType: ModelTypeDefiner#ModelType,
  basedOn: OID,
  props: PropMap
)(implicit
  val ecology: Ecology
  ...
  def getModel(implicit state: SpaceState): Thing = {
    getModelOpt(state).getOrElse(
      throw new Exception(s"Trying to fetch Model $basedOn for Model Type $modelType, but it doesn't seem to exist!")
    )
  }
  ...
  def getPropVal[VT, CT](prop: Property[VT, _])(implicit state: SpaceState): QValue = {
    localPropVal(prop).getOrElse {
      getModelOpt.map(_.getPropVal(prop)).getOrElse {
        modelType.interface[querki.ql.QL].WarningValue(
          s"Couldn't find Property ${prop.displayName} on the received value"
        )
      }
    }
  }
...
trait ModelTypeDefiner { self: EcologyMember =>
  ...
  case class ModelType(
    ...
    typeProps: PropMap
       ...
       with PTypeBuilder[ModeledPropertyBundle, SimplePropertyBundle]
       with ModelTypeBase {
    def this(
      t: OID,
      b: OID,
      tp: PropMap
    ) = this(t, SystemIds.systemOID, querki.core.MOIDs.UrTypeOID, b, tp)

    def copy(
      newModelId: OID,
      newProps: PropMap
    ) = new ModelType(t, spaceId, newModelId, basedOn, newProps)
        ...
        case _ => {
          ...
          val result = (Future.successful(Wikitext.empty) /: propInfo) { (current, pair) =>
            ...
            val propText = propOpt match {
              case Some(prop) => {
                propVal.wikify(context, displayOpt, lexicalThing).map {
                  Wikitext("<dt>" + prop.displayName + "</dt><dd>") + _ + Wikitext("</dd>")
                }
              }
