...
class DeriveNameModule(e: Ecology) extends QuerkiEcot(e) with DeriveName with NameUtils {
  ...
  private object NameDeriver extends Contributor[TCRReq, TCRReq] {
    ...
    def updateWithDerived(
      evt: ThingChangeRequest,
      displayName: String,
      kicker: Int = 0
    ): ThingChangeRequest = {
        ...
        evt.thingOpt match {
          ...
          case _ => {
            ...
            if (newDisplayNameOpt.isEmpty || newDisplayNameOpt == evt.thingOpt.flatMap(t => getDisplayName(t.props))) {
              ...
              def evtWithExistingName = existingNameOpt.map(existingName =>
                evt.copy(newProps = evt.newProps + NameProp(existingName))
              ).getOrElse(evt)
   ...
   */
  def filterNameIfDerived(
    state: SpaceState,
    model: Thing,
    props: PropList,
    propPair: (Property[_, _], DisplayPropVal)
  ): Boolean = {
  ...
  lazy val deriveModel = new ThingState(
    DeriveModelOID,
    systemOID,
    RootOID,
    toProps(
  ...
  lazy val DeriveAlways = new ThingState(
    DeriveAlwaysOID,
    systemOID,
    deriveModel,
    toProps(
  ...
  lazy val DeriveInitially = new ThingState(
    DeriveInitiallyOID,
    systemOID,
    deriveModel,
    toProps(
  ...
  lazy val DeriveNever = new ThingState(
    DeriveNeverOID,
    systemOID,
    deriveModel,
    toProps(
  ...
  lazy val DeriveNameProp = new SystemProperty(
    DeriveNameOID,
    LinkType,
    ExactlyOne,
    toProps(
