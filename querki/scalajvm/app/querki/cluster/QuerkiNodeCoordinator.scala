...
class QuerkiNodeCoordinator(e:Ecology) extends PersistentActor with Requester with EcologyMember {
  ...
  def doAssign(path:NodePath, shardId:ShardId):ShardId = {
  ...
  val receiveRecover:Receive = {
    ...
    case RecoveryCompleted => {
      ...
      shardAssignments.foreach { case (path, id) =>
        ...
        context.system.actorSelection(ActorPath.fromString(path.s)).request(CheckShardAssignment(id)) onComplete { result =>
...
object QuerkiNodeCoordinator {
   ...
   */
  case class OldShardAssigned(@KryoTag(1) nodePath:ActorRef, @KryoTag(2) shard:ShardId) extends UseKryo
  case class ShardAssigned(@KryoTag(1) nodePath:NodePath, @KryoTag(2) shard:ShardId) extends UseKryo
  
  /**
   * This Shard is now available.
   */
  case class OldShardUnassigned(@KryoTag(1) nodePath:ActorRef, @KryoTag(2) shard:ShardId) extends UseKryo
  case class ShardUnassigned(@KryoTag(1) nodePath:NodePath, @KryoTag(2) shard:ShardId) extends UseKryo
   ...
   */
  case class OldShardSnapshot(@KryoTag(1) fullShards:Set[ShardId], @KryoTag(2) shardAssignments:Map[ActorPath, ShardId]) extends UseKryo
  case class ShardSnapshot(@KryoTag(1) fullShards:Set[ShardId], @KryoTag(2) shardAssignments:Map[NodePath, ShardId]) extends UseKryo
}
