 ...
 */
private[conversations] class SpaceConversationsActor(
  ecology: Ecology,
    ...
    case CurrentState(current, _) => {
      ...
      if (boot) {
        for {
          ...
          ValuesForUser(prefs) <- space.request(SpaceSubsystemRequest(
            User.Anonymous,
            state.id,
            LoadAllPropValues(NotifyComments.GetCommentNotesPref, state)
          ))
        } {
          nextId = n + 1
  ...
  def replaceNode(
    commentId: CommentId,
    convs: ThingConversations
  )(
    f: ConversationNode => ConversationNode
  ): ThingConversations = {
          ...
          withConversations(thingId) { convs =>
            convs.findNode(commentId) match {
              case Some((node, parents)) => {
                ...
                if (
                  AccessControl.hasPermission(ConvEcot.CanModerate, state, req, thingId) || req.hasIdentity(
