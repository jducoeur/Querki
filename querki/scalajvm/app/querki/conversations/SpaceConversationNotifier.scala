 ...
 */
private[conversations] class SpaceConversationNotifier(
  e: Ecology,
  ...
  override def preStart() = {
    for {
      ValuesForUser(prefs) <- spaceRouter.request(SpaceSubsystemRequest(
        User.Anonymous,
        state.id,
...
object SpaceConversationNotifier {
  ...
  def actorProps(
    ...
    initState: SpaceState,
    spaceRouter: ActorRef
  ) = Props(classOf[SpaceConversationNotifier], e, initState, spaceRouter)

  /**
   * Sent by the ThingConversationsActor when a comment is added.
   */
  case class NotifyNewComment(
    req: User,
    comment: Comment,
