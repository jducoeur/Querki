...
class EmailFunctionsImpl(info: AutowireParams)(implicit e: Ecology)
  ...
  def getUnsubOptionsFor(unsubStr: String): Future[UnsubPageInfo] = {
    val unsubInfo =
      Unsubscribe.parseUnsub(unsubStr).getOrElse(throw new Exception(s"Failed to parse unsubStr $unsubStr"))
    if (!rc.requesterOrAnon.hasIdentity(unsubInfo.user.mainIdentity.id))
      throw new Exception(
        s"Suspicious attempt by user ${rc.requesterOrAnon} to use unsub link $unsubStr with parsed User ${unsubInfo.user}!"
      )
  ...
  def unsubscribe(
    ...
    for {
      Unsubscribed(message) <-
        Email.identityEmail ? DoUnsubscribe(OID.fromTOID(identityId), notifier, OID.fromTOID(unsubId), context)
    } yield message
  }
}
