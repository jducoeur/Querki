...
class AppsFunctionsImpl(info: AutowireParams)(implicit e: Ecology)
  extends SpaceApiImpl(info, e)
     with AppsFunctions
     with AppExtractorSupport[RequestM]
     with SpaceCreator {
    ...
    (Future.successful(Map.empty[TID, AppInfo]) /: state.apps) { (fut, app) =>
      for {
        ...
        if (currentVersionNum.v > app.version.v) {
          ...
          m + (tid -> AppInfo(
            tid,
            AppState(app.version.v, app.modTime.getMillis),
            AppState(currentVersionNum.v, currentTime.getMillis)
          ))
        } else
          m
      }
    ...
    ThingId(appIdStr) match {
      case AsOID(appId) => {
        for {
          ...
          AddAppResult(exOpt, _) <- spaceRouter.requestFor[AddAppResult](SpacePluginMsg(
            user,
            state.id,
            AddApp(appId, appVersion._1, false, false)
          ))
  ...
  def setAppState(state: SpaceState): RequestM[SpaceState] = {
    val appId = state.id
    val appRef =
      context.actorOf(PersistentSpaceActor.actorProps(ecology, SpacePersistenceFactory, requester.self, appId, false))
    for {
      // ... give it its initial state...
      ThingFound(_, newState) <-
        appRef.request(SetState(user, appId, state, SetStateReason.InitialAppState, state.displayName))
      // ... shut it down again...
