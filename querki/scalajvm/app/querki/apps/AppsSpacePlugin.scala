 ...
 */
private[apps] case class AddApp(
  appId: OID,
  appVersion: SpaceVersion,
  afterExtraction: Boolean,
  replyWithState: Boolean
)

/**
 * Response from AddApp. Should be considered successful unless there is an Exception here.
 */
private[apps] case class AddAppResult(
  exceptionOpt: Option[PublicException],
  stateOpt: Option[SpaceState]
)
