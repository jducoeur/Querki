...
trait AppsPure extends ModelPersistence with ModelTypeDefiner with EcologyMember {
  ...
  def addFilledAppPure(filledApp:SpaceState, shadowMapping:Map[OID, OID], modTime:DateTime)(state:SpaceState):SpaceState = {
    // First, add the Apps themselves to the Space
    val stateWithApps = state.copy(apps = state.apps :+ filledApp, appInfo = state.appInfo :+ (filledApp.id, filledApp.version))
    
    val shadowRoots:Set[OID] = 
      stateWithApps.
      things.
      values.
      filter(_.ifSet(Apps.ShadowFlag)(stateWithApps)).
      map(Apps.getShadowedThing(_)(stateWithApps)).
      map(_.id).
      toSet
   ...
   */
  def addAppPure(rawApp:SpaceState, addedApps:Map[OID, SpaceState], existingApps:Map[OID, SpaceState], shadowMapping:Map[OID, OID], modTime:DateTime)(state:SpaceState):SpaceState = {
    val (filledApp, allApps) = fillInApps(rawApp, addedApps, existingApps)
    addFilledAppPure(filledApp, shadowMapping, modTime)(state)
  }
  ...
  def evolveStateApps(stateOpt:Option[SpaceState]):PartialFunction[SpaceEvent, SpaceState] = {
    ...
    case DHAddApp(req, modTime, appState, parentApps, shadowMapping) => {
      implicit val s = stateOpt.get
      val newApps = parentApps.map(rehydrate(_))
      val addedApps = (Map.empty[OID, SpaceState] /: newApps) { (acc, app) =>
        acc + (app.id -> app)
      }
      addAppPure(rehydrate(appState), addedApps, s.allApps, shadowMapping, modTime)(s)
    }
