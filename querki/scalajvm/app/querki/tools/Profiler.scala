...
class ProfilerEcot(e: Ecology) extends QuerkiEcot(e) with Profiler {
  ...
  private case class ProfileRecord(
    name: String,
    runs: Int,
    time: Duration
  )
...
private[tools] case class ProfileHandleImpl(
  ecot: ProfilerEcot,
  name: String
) extends ProfileHandle {
...
private[tools] case class ProfileInstanceImpl(
  handle: ProfileHandleImpl,
  startTime: DateTime,
  namePlus: String
) extends ProfileInstance {
