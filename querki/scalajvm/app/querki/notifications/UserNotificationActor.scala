...
import querki.identity.{UserId, UserRouteableMessage}
...
class UserNotificationActor(val ecology: Ecology)
  extends Actor
     with Stash
     with Requester
     with EcologyMember
...
object UserNotificationActor {
   ...
   */
  case class NewNotification(
    userId: UserId,
    note: Notification
  ) extends UserRouteableMessage {
