...
case class QLParam(
  ...
  resolved: Option[QLContext] = None
) {
...
case class QLCall(
  name: QLName,
  methodName: Option[QLName],
  params: Option[Seq[QLParam]],
  collFlag: Option[String]
) extends QLStage(collFlag) {
...
private[ql] case class QLBindingDef(
  n: String,
  func: Option[QLPhrase] = None,
  ...
  def reconstructString = {
    func match {
      case Some(f) =>
        s"_def $$$n${params.map(_.map("$" + _).mkString("(", ", ", ")")).getOrElse("")} = ${f.reconstructString}"
      case _ => "+$" + n
    }
  }
}
