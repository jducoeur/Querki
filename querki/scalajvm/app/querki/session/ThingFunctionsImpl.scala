...
import querki.api.{AutowireParams, SpaceApiImpl, ThingFunctions, UnknownThingException}
import querki.core.QLText
import querki.data._
import querki.pages.ThingPageDetails
import querki.spaces.messages.{DeleteThing, ThingError, ThingFound}
...
class ThingFunctionsImpl(info: AutowireParams)(implicit e: Ecology) extends SpaceApiImpl(info, e) with ThingFunctions {
  ...
  def getThingPage(
    thingId: TID,
    renderPropIdOpt: Option[TID]
  ): Future[ThingPageDetails] = getThingPageProfiler.profileFut {
    withThing(thingId) { thing =>
  ...
  def evaluateQL(
    thingId: TID,
    ql: String
  ): Future[Wikitext] = withThing(thingId) { thing =>
    implicit val r = rc
    implicit val s = state
  ...
  def evaluateQLWithContext(
    ...
    lexicalOpt: Option[TID],
    ql: String
  ): Future[Wikitext] = withThing(typeId) { case pt: PType[_] =>
    implicit val r = rc
    implicit val s = state
      ...
      thing.getPropOpt(propId) match {
        case Some(pv) => {
          ...
          tryType(Core.YesNoType)(BoolV).orElse(
            tryType(Core.TextType) { vs => TextV(vs.map(_.text)) }
          ).orElse(
            tryType(Core.LargeTextType) { vs => TextV(vs.map(_.text)) }
          ).orElse(
  ...
  def getPropertyDisplay(
    thingId: TID,
    propIdStr: TID
  ): Future[Option[Wikitext]] = withThing(thingId) { thing =>
    implicit val s = state
    implicit val r = rc
  ...
  def getAllProperties(): SpaceProps = {
    ...
    def getPropsForSpace(
      ...
      SpaceProps(
        ...
        space.apps.map(getPropsForSpace(_, false)) ++ (if (withSystem) Seq(getPropsForSpace(space.system.get, false))
                                                       else Seq.empty)
      )
    }
  ...
  def getAllTypes(): Future[AllTypeInfo] = {
    ...
    val spaceTypes =
      state.allTypes.values.filterNot(_.ifSet(Core.InternalProp)).filterNot(_.ifSet(Basic.DeprecatedProp)).filterNot(
        typ => typ.isInstanceOf[querki.types.ModelTypeBase] && !typ.ifSet(Basic.ExplicitProp)
      )
    ...
    Future.sequence(
      ...
      AllTypeInfo(
        ...
        Seq.empty,
        spaceTypes.map(typ =>
          TypeInfo(typ, typ.linkName, typ.displayName, typ.firstOpt(Editor.PreferredCollectionProp).map(_.toTID))
        ).toSeq,
        modelInfo
      )
    }
