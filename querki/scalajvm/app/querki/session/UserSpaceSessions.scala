...
private[session] class UserSpaceSessions(
  ...
  override def childrenUpdated() = {
    ...
    state.foreach(s =>
      monitor ! SpaceMonitorEvent(spaceId, s.displayName, SystemManagement.clusterAddress, nChildren, s.spaceSize)
    )
  }
  ...
  def createChild(key: User): ActorRef = {
    ...
    context.actorOf(
      OldUserSpaceSession.actorProps(ecology, spaceId, key, spaceRouter, persister, timeSpaceOps).withDispatcher(
        "session-dispatcher"
      ),
      key.id.toString
    )
