 ...
 */
class DataModelAccessEcot(e: Ecology)
  extends QuerkiEcot(e)
     with DataModelAccess
  ...
  lazy val CopyIntoInstances = new SystemProperty(
    CopyIntoInstancesOID,
    YesNoType,
    ExactlyOne,
    toProps(
      ...
      Details(
        """Usually, Querki's Properties are inherited from the Model to the Instance -- that is, if the
    ...
    for {
      ...
      linkProps = definingProp match {
        ...
        case _ => {
          ...
          inv.state.propList.filter(prop => prop.pType == LinkType).
          // filter this to just the Properties with the right Restricted To:
          filter(prop => prop.getPropVal(Links.LinkModelProp)(inv.state).contains(LinkType, modelOID)).map(
            _.confirmType(LinkType).get
          )
        }
      }
  ...
  lazy val AllRefsMethod = new InternalMethod(
    AllRefsOID,
    toProps(
      setName("_allRefs"),
      Categories(DataModelTag),
      ...
      Details(
        """`_allRefs` is more or less exactly the combination of `_refs` and `_tagRefs`. It is
  ...
  lazy val UsingSpace = new InternalMethod(
    UsingSpaceOID,
    toProps(
      setName("_usingSpace"),
      Categories(DataModelTag),
  ...
  lazy val AllTypesMethod = new InternalMethod(
    AllTypesMethodOID,
    toProps(
      setName("_allTypes"),
      Categories(DataModelTag),
    ...
    extends InternalMethod(
      ...
      toProps(
        ...
        Signature(
          ...
          returns = (
            LinkType,
            "All of the direct children of that Model in that Space. This may include sub-Models, if there are any, as well as Instances."
          )
        )
      )
    ) {
    ...
    override def qlApply(inv: Invocation): QFut = {
      inv.paramsOpt match {
        ...
        case Some(params) => {
          ...
          Future.successful(Kind.fromName(kindName.name).map(kind => ExactlyOne(IntType(kind))).getOrElse(
            QL.WarningValue("Unknown Kind: " + kindName)
          ))
        }
  ...
  class CurrentSpaceMethod
    extends InternalMethod(
      CurrentSpaceMethodOID,
      toProps(
  ...
  lazy val IsAFunction = new InternalMethod(
    IsAFunctionOID,
    toProps(
      setName("_isA"),
      Categories(DataModelTag),
  ...
  lazy val PropsOfTypeMethod = new InternalMethod(
    PropsOfTypeOID,
    toProps(
  ...
  lazy val IsFunctionProp = new SystemProperty(
    IsFunctionOID,
    YesNoType,
    ExactlyOne,
    toProps(
  ...
  lazy val HasPropertyMethod = new InternalMethod(
    HasPropertyMethodOID,
    toProps(
  ...
  lazy val AllThingsMethod = new InternalMethod(
    AllThingsOID,
    toProps(
      setName("_allThings"),
      Categories(DataModelTag),
  ...
  lazy val AsTypeMethod = new InternalMethod(
    AsTypeMethodOID,
    toProps(
      setName("_asType"),
      Categories(DataModelTag),
  ...
  lazy val ModelFunction = new InternalMethod(
    ModelFunctionOID,
    toProps(
      setName("_model"),
      Categories(DataModelTag),
  ...
  lazy val OrphanedInstancesFunction = new InternalMethod(
    OrphanedInstancesOID,
    toProps(
  ...
  lazy val WithValueInMethod = new InternalMethod(
    WithValueInOID,
    toProps(
     ...
     * InstanceValueMap.
     */
    def createValueMap[VT](
      inv: Invocation,
      modelId: OID,
      exp: QLExp,
      expectedType: PType[VT]
    ): Future[InstanceValueMap[VT]] = {
