...
class IntrospectionEcot(e: Ecology) extends QuerkiEcot(e) with querki.core.MethodDefs {
   ...
   */
  lazy val PropAndValType = new SystemType[DisplayPropVal](
    UnknownOID,
    toProps(
      setName("Prop and Val Type"),
      Core.InternalProp(true)
    )
  ) with SimplePTypeBuilder[DisplayPropVal] {
  ...
  lazy val foreachPropertyMethod = new InternalMethod(
    ForeachPropertyMethodOID,
    toProps(
    ...
    override def qlApply(invIn: Invocation): QFut = {
      ...
      for {
        ...
        result <- {
          if (inv.numParams == 0) inv.wrap(propAndVal) else inv.processParam(0, inv.context.next(propAndVal))
        }
      } yield result
    }
  }

  lazy val valMethod = new InternalMethod(
    ValMethodOID,
    toProps(
      setName("_val"),
      Categories(DataModelTag),
  ...
  lazy val rawValMethod = new InternalMethod(
    RawValMethodOID,
    toProps(
      setName("_rawVal"),
  ...
  lazy val propMethod = new InternalMethod(
    PropMethodOID,
    toProps(
      setName("_prop"),
      Categories(DataModelTag),
  ...
  lazy val definedOnMethod = new InternalMethod(
    DefinedOnMethodOID,
    toProps(
      setName("_definedOn"),
      Categories(DataModelTag),
    ...
    override def qlApply(inv: Invocation): QFut = {
      for {
        ...
        case Some(on) => on match {
            ...
            case bundle @ querki.types.ModeledPropertyBundle(mt, _, _) =>
              ExactlyOne(mt(querki.types.SimplePropertyBundle(bundle.props.toSeq: _*)))
          }
        case None => Core.QNone
      }
    }
  ...
  lazy val isInheritedMethod = new InternalMethod(
    IsInheritedMethodOID,
    toProps(
      setName("_isInherited"),
      Categories(DataModelTag),
      Summary(
        "Says whether the current Property's value is locally defined or was inherited from the Model, inside of _foreachProperty"
      )
    )
  ) {
