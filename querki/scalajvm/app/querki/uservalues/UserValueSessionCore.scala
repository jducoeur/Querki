    ...
    if (!rawUvProps.isEmpty) {
      rawState.anything(thingId) match {
        case Some(thing) => {
          ...
          val uvProps = rawUvProps.filter { case (propId, v) =>
            AccessControl.hasPermission(UserValues.UserValuePermission, rawState, identity.id, thing.id)
          }
          if (!uvProps.isEmpty) {
