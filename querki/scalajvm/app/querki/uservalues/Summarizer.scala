...
trait Summarizer[UVT, VT] {
  ...
  def addToSummary(
    tid: OID,
    fromProp: Property[_, _],
    prop: Property[VT, _],
    previous: Option[QValue],
    current: Option[QValue]
  )(implicit
    state: SpaceState
  ): QValue
...
trait SummarizerDefs { self: QuerkiEcot =>
   ...
   * for Summarizers.
   */
  abstract class SummarizerBase[UVT, VT](
    tid: OID,
    pf: PropMap
  ) extends PType[VT](tid, SystemIds.systemOID, MOIDs.SummarizerBaseOID, pf)
       with Summarizer[UVT, VT]
    ...
    def viaPath(
      bundleOpt: Option[PropertyBundle],
      path: PropPath[_, _]
    )(implicit
      state: SpaceState
    ): Option[UVT] = {
    ...
    def addToSummary(
      ...
      val resultViaPath = fromProp.pType match {
        case mt: ModelTypeBase => {
          ...
          } yield ExactlyOne(ElemValue(
            doAddToSummary(tid, summarizesProp, prop, viaPath(previousBundle, path), viaPath(currentBundle, path)),
            this
          ))
        }
        case _ => None
      ...
      resultViaPath.getOrElse(ExactlyOne(ElemValue(
        doAddToSummary(tid, fromProp, prop, castToUVT(previous), castToUVT(current)),
        this
      )))
    }
      ...
      def recalculateOneThing(
        ...
        pathOpt match {
          case Some((mt, path, summarizesProp)) => {
            ...
            doRecalculateOneThing(tid, summarizesProp, prop, uvts).map(result =>
              (tid, ExactlyOne(ElemValue(result, this)))
            )
          }
          case None => {
            val vs = values.map(_.v)
   ...
   */
  class DiscreteSummarizer[UVT](
    tid: OID,
    userType: PType[UVT],
    p: PropMap
  ) extends SummarizerBase[UVT, DiscreteSummary[UVT]](tid, p) {

    def doAddToSummary(
      tid: OID,
      fromProp: Property[_, _],
      prop: Property[DiscreteSummary[UVT], _],
      previous: Option[UVT],
      current: Option[UVT]
    )(implicit
      state: SpaceState
    ...
    def doDeserialize(ser: String)(implicit state: SpaceState): DiscreteSummary[UVT] = {
      ...
      val pairs =
        if (pairStrs.length > 0 && pairStrs(0).length() > 0) pairStrs.map { pairStr =>
