...
case class ChangeResult(
  events: List[SpaceEvent with UseKryo],
  changedThing: Option[OID],
  resultingState: SpaceState
)
...
abstract class SpaceCore[RM[_]](val rtc: RTCAble[RM])(implicit val ecology: Ecology)
  extends SpaceMessagePersistenceBase
     with SpaceAPI[RM]
...
//    val dhApps = currentState.allApps.values.toSeq.map(dh(_))
  ...
  def updateStateCore(newState: SpaceState): SpaceState = {
    ...
    val filledState =
      SpaceChangeManager.updateStateCache(CacheUpdate(None, _currentState, newState)).current.copy(version =
        SpaceVersion(lastSequenceNr))
  ...
  def updateState(
    newState: SpaceState,
    events: Option[List[SpaceEvent]] = None
  ): SpaceState = {
  ...
  def canCreate(
    ...
    state: SpaceState
  ): Boolean = AccessControl.canCreate(state, who, modelId)
  ...
  def canDesign(
    ...
    state: SpaceState
  ): Boolean = AccessControl.canDesign(state, who, modelId)
  ...
  def canEdit(
    ...
    state: SpaceState
  ): Boolean = AccessControl.canEdit(state, who, thingId)

  /**
   * Goes through the Props, and figures out what is actually changing.
   */
  def changedProperties(
    oldProps: PropMap,
    newProps: PropMap
  ): Seq[OID] = {
  ...
  // general principles.
  def canChangeProperties(
    who: User,
    changedProps: Seq[OID],
    oldThingOpt: Option[Thing],
    newProps: PropMap
  )(implicit
    state: SpaceState
  ): Unit = {
    val failedProp = changedProps.find(!AccessControl.canChangePropertyValue(state, who, _))
    failedProp.map { propId =>
      throw new PublicException("Space.modifyThing.propNotAllowed", state.anything(propId).get.displayName)
    }
  }
  ...
  def doCreate(
    ...
    thingIdRM.flatMap { thingId =>
      ...
      rtc.successful(ChangeResult(
        List(msg),
        Some(thingId),
        createPure(creator, kind, thingId, modelId, props, createTime)(state)
      ))
    }
  }
      ...
      offerChanges(who, modelIdOpt, Some(oldThing), oldThing.kind, rawNewProps, changedProps)(state).flatMap { tcr =>
        ...
        if (!canEdit(who, thingId)) {
          ...
          rtc.successful(ChangeResult(
            List(msg),
            Some(thingId),
            modifyPure(thingId, oldThing, modelIdOpt, newProps, replaceAllProps, modTime)(state)
          ))
        }
      }
    }
  ...
  def deleteThing(
    ...
    if (
      !DataModel.isDeletable(oldThing)(state)
   ...
   */
  def runChanges(
    funcs: Seq[SpaceState => RM[ChangeResult]]
  )(
    state: SpaceState
  ): RM[(List[ChangeResult], SpaceState)] = {
  ...
  def runAndSendResponse(
    ...
    result.map { case (changes, finalState) =>
      changes.lastOption.foreach(change =>
        change.changedThing.foreach { oid =>
  ...
  /**
   * Spewage to print during initialization for *all* Spaces.
   *
  ...
  def recoverPersistence: Receive = {
    case SnapshotOffer(metadata, dh: DHSpaceState) => {
      handlingInitExceptions("Old Snapshot") {
    ...
    case SnapshotOffer(metadata, SpaceSnapshot(dh, dhApps)) => {
      handlingInitExceptions("New Snapshot") {
    ...
    case RecoveryCompleted => {
      handlingInitExceptions("RecoveryCompleted") {
    ...
    if (_enhancedState.isEmpty) {
      ...
      _enhancedState = Some(
        _pubState.map { ps =>
          ...
          val filled =
            SpaceChangeManager.updateStateCache(CacheUpdate(None, Some(currentState), enhanced)).current.copy(version =
  ...
  val normalReceiveCommand: Receive = {
      ...
      runAndSendResponse("changeProps", localCall, modifyAndPublishIfNeeded, pub)(fullState).map { changes =>
        ...
        changes.last.changedThing.map { lastThingId =>
          if (lastThingId == spaceId) {
            ...
            def disp(state: SpaceState) =
              Basic.DisplayNameProp.firstOpt(state.props).map(_.raw.toString).getOrElse(linkName(state))

            if (
              !NameUtils.equalNames(linkName(finalState), linkName(initialState))
    ...
    case ModifyThing(who, spaceId, thingId, modelId, newProps, localCall) => {
      val (pub, fullState) =
        if (isPublishable(thingId))
          ...
          (false, currentState)
      runAndSendResponse("modifyThing", localCall, modifyThing(who, thingId, Some(modelId), newProps, true), pub)(
        fullState
      )
    }
    ...
    case ChangeModel(who, spaceId, thingId, newModelId, localCall) => {
      runAndSendResponse(
        "changeModel",
        localCall,
        modifyThing(who, thingId, Some(newModelId), emptyProps, false),
        false
      )(currentState)
    }
