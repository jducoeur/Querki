...
case class MySpaces(
  ownedByMe: Seq[SpaceDetails],
  memberOf: Seq[SpaceDetails]
) extends ListMySpacesResponse
...
case class CreateSpace(
  requester: User,
  name: String,
  initialStatus: SpaceStatusCode = StatusNormal
) extends SpaceMgrMsg

case class ChangeSpaceStatus(
  spaceId: OID,
  newStatus: SpaceStatusCode
) extends SpaceMgrMsg
 ...
 */
case class ChangeProps(
  req: User,
  space: OID,
  id: ThingId,
  changedProps: PropMap,
  localCall: Boolean = true
) extends SpaceMessage(req, space)

case class DeleteThing(
  req: User,
  space: OID,
  thing: ThingId
) extends SpaceMessage(req, space)
 ...
 */
case class SpaceSubsystemRequest(
  req: User,
  space: OID,
  payload: SpaceMessagePayload
) extends SpaceMessage(req, space)
 ...
 */
case class BeginProcessingPhoto(
  req: User,
  space: OID,
...
case class InviteRequest(
  rc: RequestContext,
  emails: Seq[querki.email.EmailAddress],
  collabs: Seq[OID]
...
case class ReplacePerson(
  guestId: OID,
  actualIdentity: PublicIdentity
) extends SpaceMembersBase
...
case class ThingFound(
  id: OID,
  state: SpaceState
) extends ThingResponse

case class ThingError(
  ex: PublicException,
  stateOpt: Option[SpaceState] = None
) extends ThingResponse
