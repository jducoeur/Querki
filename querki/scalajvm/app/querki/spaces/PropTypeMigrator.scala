...
case class TypeChangeInfo(
  typeChanged: Boolean,
  newType: PType[Any] with PTypeBuilder[Any, Any],
  serializedValues: Map[Thing, String],
  prop: Property[_, _]
) {

  def finish(
    newProp: Property[_, _],
    stateIn: SpaceState,
    updateState: (SpaceState, Option[SpaceMessage]) => Unit
  )(implicit
    e: Ecology
  ): Unit = {
    if (typeChanged) {
