...
class MySQLImport(
    ...
    (stateIn /: db.tables.values) { (state, table) =>
      val mainPropPairs = table.columns.values.map { col =>
        colMap.get(col.col).map { prop =>
          val default: QValue =
            col.defaultOpt.filter(_ != NullVal).flatMap(v => buildQValue(col, prop, v)).getOrElse(prop.default(stateIn))
          (prop.id, default)
        }
      }.flatten.toSeq
    ...
    (stateIn /: db.tables.values) { (tblState, table) =>
      (tblState /: table.constraints) { (constState, constraint) =>
        ...
        if (!foreignTbl.primaryKey.isDefined || !(foreignTbl.primaryKey.get == foreignCol.name)) {
          throw new Exception(
            s"We can currently only deal with Constraints to primary keys! Failed constraint ${table.name}.${foreignCol.name}"
          )
        }
        val prop = colMap(localCol.col)
        if (prop.pType != Core.LinkType) {
          throw new Exception(
            s"Somehow wound up with constraint $constraint not pointing to a Link Property! Col = $localCol; prop = $prop (${prop.cType} of ${prop.pType})"
          )
        }
  ...
  private def buildInstance(
    db: MySQLDB,
    table: MySQLTable,
    primaryOpt: Option[ColumnName],
    tIn: ThingState,
    cols: Seq[MySQLColumn],
    props: Seq[Option[AnyProp]],
    vals: Seq[SQLVal[_]]
  ): ThingState = {
    if (cols.isEmpty)
      tIn
