...
object MySQLProcess {
  ...
  case class MySQLTable(
    name: TableName,
    columns: Map[ColumnName, MySQLColumn],
    columnOrder: Seq[ColumnName],
    primaryKey: Option[ColumnName],
    constraints: Seq[SQLConstraint],
    data: Option[MySQLData] = None
  )
  case class MySQLRow(vs: Seq[SQLVal[_]])

  case class MySQLColumn(
    col: ColumnInfo,
    generateProp: Boolean = true,
    rawConstraint: Option[SQLConstraint] = None
  ) {
