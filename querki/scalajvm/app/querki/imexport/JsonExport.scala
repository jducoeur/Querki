...
class JsonExport(implicit val state: SpaceState, val ecology: Ecology) extends EcologyMember {
   ...
   * they use.
   */
  def jsonifyIfPropBundle(implicit qv: QValue, multi: Boolean, seen: Set[OID]): Option[JsValue] = {
  ...
  def jsonifyIfLink(implicit qv: QValue, multi: Boolean, seen: Set[OID]): Option[JsValue] = {
  ...
  def jsonifyInternal(implicit qv: QValue, multi: Boolean, seen: Set[OID]): JsValue = {
    val resultOpt = 
   ...
   */
  def jsonify(qv: QValue, pretty: Boolean = false): String = {
