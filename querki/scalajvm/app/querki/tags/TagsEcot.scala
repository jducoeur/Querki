...
class TagsEcot(e: Ecology)
  ...
  type TagMapT = scala.collection.mutable.HashMap[String, scala.collection.mutable.Set[Thing]]
    with scala.collection.mutable.MultiMap[String, Thing]
  type TagRefCacheT = scala.collection.concurrent.Map[OID, TagMapT]

  def trCache(implicit state: SpaceState) =
    state.fetchOrCreateCache(cacheKey, { scala.collection.concurrent.TrieMap.empty[OID, TagMapT] }).asInstanceOf[
      TagRefCacheT
    ]
  ...
  def nameCache(implicit state: SpaceState) =
    state.fetchOrCreateCache(nameCacheKey, { scala.collection.concurrent.TrieMap.empty[String, String] }).asInstanceOf[
      NameCacheT
    ]
   ...
   */
  def cachedTagRefsFor(
    rawProp: Property[_, _]
  )(implicit
    state: SpaceState
  ): scala.collection.Map[String, scala.collection.mutable.Set[Thing]] = {
    // Note that the trCache is indexed by Property OID:
    trCache.getOrElseUpdate(
      rawProp.id, {
        def addBindings[VT](
          prop: Property[_, _],
          pt: PType[VT],
          ...
          val tagMap = new scala.collection.mutable.HashMap[String, scala.collection.mutable.Set[Thing]]
            with scala.collection.mutable.MultiMap[String, Thing]
          for {
  ...
  lazy val TagSetType = new NameTypeBase(
    TagSetOID,
    toProps(
    ...
    override def renderProperty(
      ...
      Some(QL.process(
        querki.core.QLText("""These tags are currently being used:
  ...
  def tagPropError(
    tagName: String,
    propName: String
  ): PublicException = new PublicException("Tags.noProp", tagName, propName)
  ...
  class TagThingOps(tag: TagThing) extends ThingOps(tag)(ecology) {
    ...
    override def nameOrComputedWiki(
      implicit
      request: RequestContext,
      state: SpaceState
    ): Future[Wikitext] = Future.successful(Wikitext(name))

    override def nameOrComputed(
      implicit
      rc: RequestContext,
      state: SpaceState
    ) = Future.successful(DisplayText(name))

    override def unsafeNameOrComputed(
      implicit
      rc: RequestContext,
      state: SpaceState
    ...
    override def render(
      ...
      propAndValOpt.map(pv => pv.render(nameAsContext)).getOrElse(Future.successful(
        Wikitext(name + " doesn't exist yet. [Click here to create it.](edit?thingId=" + SafeUrl(name) + ")")
      ))
    }
  }
  ...
  lazy val ShowUnknownProp = new SystemProperty(
    ShowUnknownOID,
    LargeTextType,
    ExactlyOne,
    toProps(
      ...
      Details(
        """In Querki, it is entirely legal to refer to the name of something you haven't written yet --
  ...
  lazy val IsReifiedTagProp = new SystemProperty(
    IsReifiedTagOID,
    YesNoType,
    Optional,
    toProps(
  ...
  lazy val TagRefsMethod = new InternalMethod(
    TagRefsOID,
    toProps(
      setName("_tagRefs"),
      Categories(TagsTag),
  ...
  lazy val TagsForPropertyMethod = new InternalMethod(
    TagsForPropertyOID,
    toProps(
  ...
  lazy val resolveTagsMethod = new InternalMethod(
    ResolveTagsOID,
    toProps(
