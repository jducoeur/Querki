...
import querki.values.{ElemValue, PropAndVal, QLContext, QValue, SpaceState, StateCacheKey}
  ...
  with TextTypeBasis with IntTypeBasis with LinkUtils with NameUtils with NameTypeBasis with TypeCreation
{
   ...
   */
  lazy val AbstractOverModel = new ThingState(AbstractOverModelOID, systemOID, RootOID,
    toProps(
   ...
   */
  lazy val AbstractOverReceived = new ThingState(AbstractOverReceivedOID, systemOID, AbstractOverModelOID,
    toProps(
   ...
   */
  lazy val ImplementationModel = new SystemProperty(ImplementationModelOID, YesNoType, ExactlyOne,
    toProps(
      setName("_implementationModel"),
      setInternal))
  
  lazy val ImplementsFunctionProp = new SystemProperty(ImplementsFunctionPropOID, LinkType, ExactlyOne,
    toProps(
      setName("Implements Abstract Function"),
      AppliesToKindProp(Kind.Property),
      setInternal,
      Summary("This should only be applied to Function Implementations, and says which Abstract Function this implements")))
  
  lazy val AbstractOverProp = new SystemProperty(AbstractOverPropOID, LinkType, ExactlyOne,
    toProps(
  ...
  lazy val ImplementsTypesProp = new SystemProperty(ImplementsTypePropOID, LinkType, QList,
    toProps(
  ...
  def computeImplMap(state:SpaceState):FunctionImpls = {
    val impls = state.allProps.values.filter(_.model == querki.core.MOIDs.ImplementationModelOID)
