...
abstract class Collection(
  ...
  m: OID,
  pf: PropMap
) extends Thing(i, s, m, Kind.Collection, pf, querki.time.epoch) {
  ...
  final def deserialize(
    ...
    state: SpaceState
  ): QValue = makePropValue(doDeserialize(ser, elemT), elemT)
...
//  final def serialize(v:PropValue, elemT:pType):String = doSerialize(v.cv, elemT)
  ...
  def debugRender(
    ...
    v: implType,
    elemT: pType
  ): String = v.map(elemT.debugRender(context)(_)).mkString(", ")
