...
class CollectionSerializationTests extends QuerkiTests {
  ...
  class CollSpace extends CommonSpace {
    ...
    val oneThing =
      new SimpleTestThing("One Thing", singleTextProp("hello"), singleTagProp("floob"), singleLinkProp(linkableThing))
    val emptyOneThing = new SimpleTestThing("Empty One Thing", singleTextProp(""))

    val optThing =
      new SimpleTestThing("Opt Thing", optTextProp("hello"), optTagProp("floob"), optLinkProp(linkableThing))
    val emptyOptThing = new SimpleTestThing("Empty Opt Thing", optTextProp(), optTagProp(), optLinkProp())

    val listThing =
      new SimpleTestThing("List Thing", listTagsProp("foo", "bar"), listLinksProp(linkable2, linkableThing))
    val emptyListThing = new SimpleTestThing("Empty List Thing", listTagsProp(), listLinksProp())
  }
