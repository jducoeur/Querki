...
class SpaceToExtract(implicit e: Ecology) extends AppableSpace {
  ...
  val instance31 = addThing("Instance 3 1", model3, propOfModelType())
  val instance32 = addThing("Instance 3 2", model3)

  val instance33 = addThing(
    "Instance 3 3",
    model3,
    propOfModelType(SimplePropertyBundle(
      textProp("3 3 Value"),
...
class ExtractAppTests extends QuerkiTests {
  ...
  def testChildSpace(
    original: SpaceToExtract,
    space: SpaceCoreSpaceBase,
    appState: SpaceState
  ) = {
...
//      QLog.spewState(newState)
      ...
      val localType =
        newState.localFrom(AsName("Type Model"), newState.types).get.asInstanceOf[ModelTypeDefiner#ModelType]
      ...
      val localChild32 = newChild.addThing(
        "Local Instance 3 2",
        localModel3,
        localComplexProp(QList(localType(SimplePropertyBundle(
