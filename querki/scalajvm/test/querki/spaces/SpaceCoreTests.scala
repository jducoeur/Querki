...
class TestSpaceCore(
  ...
  e: Ecology
) extends SpaceCore[TCIdentity](TestRTCAble)
  ...
  def notifyPublished(
    ...
    state: SpaceState
  ): TCIdentity[PublishedAck] = {
    TestRTCAble.successful(PublishedAck())
  }

  def selfDestruct(): Unit = {
    // So far, we don't do anything with this. We might want to test this more properly in the future.
  }
}
...
abstract class SpaceCoreSpaceBase()(implicit val ecology: Ecology) extends TestSpace with TestConversations {
  ...
  def !(msg: AnyRef): Option[AnyRef] = {
    msg match {
      ...
      case msg @ SpaceSubsystemRequest(_, _, payload: querki.conversations.messages.ConversationMessage) =>
        routeToConv(msg)
  ...
  def addProperty[VT, RT](
    tpe: PType[VT] with PTypeBuilder[VT, RT],
    coll: Collection,
    name: String
  ): PropRecord[VT, RT] = {
    val oid = addSomething(name, Kind.Property, UrPropOID, Core.CollectionProp(coll), Core.TypeProp(tpe))
  ...
  def addSimpleModel(
    ...
    addThing(name, SimpleThingOID, (Core.IsModelProp(true) +: propList): _*)
  }
