...
class ComplexSpace(implicit ec: Ecology) extends CommonSpace with ModelTypeDefiner {
  ...
  val modelForType = new SimpleTestThing(
    "Model for Type",
    numberProp(0),
    textProp(""),
  ...
  val thingWithComplex = new SimpleTestThing(
    "My Complex Thing",
  ...
  val metaModel = new SimpleTestThing(
    "Meta Model",
    propOfModelType(SimplePropertyBundle(
      numberProp(42),
  ...
  val metaThing = new SimpleTestThing(
    "Top level Thing",
  ...
  val nodeModel =
    new TestThing(nodeModelId, "Tree Node Model", Core.IsModelProp(true), leftProp(), rightProp(), nodeId())

  val myTree = new TestThing(
    "My Tree",
    nodeModel,
...
//    }
  ...
  "A nested Model Type" should {
    ...
    "be able to access properties from its Thing" in {
      ...
      // at the top:
      pql("""[[Top Level Thing -> Meta Property -> _first -> Complex Prop -> Referencing]]""") should equal(
        "From the Top"
      )
    }
    ...
    "be able to access its Thing from _edit" in {
      ...
      pql("[[Top Level Thing -> Meta Property -> _first -> Complex Prop._edit]]") should include(
        "Reference from subModel: From the Top"
      )
    }
  }
