...
class IntrospectionTests extends QuerkiTests {
  ...
  class TSpace extends CommonSpace with ModelTypeDefiner {
    ...
    val modelWithOpt = new SimpleTestThing(
      "Model with Opt",
      prop1(1),
      prop2(2),
      optProp(Core.QNone),
    ...
    val modelWithOpts = new SimpleTestThing(
      "Model with Opts",
      optProp(0),
      optProp2(0),
      optProp3(0),
    ...
    val thingWithOpts =
      new TestThing("Thing with Opts", modelWithOpts, optProp(Core.QNone), optProp2(4), optProp3(Core.QNone))

    // props intentionally not quite in either alphabetical or numeric order:
    val bottomModel = new SimpleTestThing(
      "Bottom Model",
      prop1(1),
      prop2(2),
      prop4(4),
    ...
    val complexThing =
      new TestThing("My Complex Thing", complexThingModel, propOfModelType(SimplePropertyBundle(prop3(9))))
  ...
  "_foreachProperty" should {
    "work for a simple Model Value" in {
      ...
      pql(
        """[[My Complex Thing -> Complex Prop -> _foreachProperty(""Name: [[_prop -> Link Name]]; Value: [[_val]]"") -> _bulleted]]"""
      ) should
        equal("""
            |<ul>
            |<li class="_bullet">
    ...
    "work with a non-quoted Expression" in {
      ...
      pql(
        """[[Thing with Opt -> _foreachProperty(_if(_val -> _isNonEmpty, ""[[_prop -> Link Name]]: [[_val]]""))]]"""
      ) should
        equal("""
            |First Prop: 1
            |Second Prop: 2""".stripReturns)
      pql(
        """[[My Complex Thing -> Complex Prop -> _foreachProperty(_if(_isNonEmpty, ""[[_prop -> Link Name]]: [[_val]]""))]]"""
      ) should
        equal("""
            |First Prop: 1
            |Second Prop: 2
  ...
  "_definedOn" should {
    "work properly" in {
      ...
      pql(
        """[[Bottom Model._instances -> _foreachProperty(""Name: [[_prop -> Link Name]]; Value: [[_val]]; On: [[_definedOn -> Link Name]]"") -> _bulleted]]"""
      ) should
        equal("""
            |<ul>
			|<li class="_bullet">
