...
class ConversationEventPersistenceTests(env: PersistEnv)
    ...
    val resps = respIn match {
      case head :: tail => {
        head.copy(comment = head.comment.copy(primaryResponse = true, responseTo = Some(commentId))) +:
          tail.map(tailResp => tailResp.copy(comment = tailResp.comment.copy(responseTo = Some(commentId))))
      }
      case Nil => Seq.empty
    }
  ...
  val testConvs = ThingConversations(Seq(
    comment(
      owner,
      "Let's have us a conversation, chaps, shall we?",
      Seq(
        comment(
          m1,
          "Why yes, good fellow, let us do so",
          Seq(
