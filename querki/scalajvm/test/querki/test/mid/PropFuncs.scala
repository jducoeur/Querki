...
trait PropFuncs {
   ...
   */
  def makeProperty(name: String, coll: ThingInfo, tpe: TID): TestOp[TID] = {
    for {
   ...
   */
  def makeProperty(name: String, collOp: TestOp[ThingInfo], tpeOp: TestOp[TID]): TestOp[TID] = {
    for {
      coll <- collOp
      tpe <- tpeOp
   ...
   * Property's value.
   */
  def addProperty(thingId: TID, propId: TID): TestOp[PropEditInfo] = {
    for {
   ...
   */
  def addAndSetProperty[T: Saveable](thingId: TID, propId: TID, v: T): TestOp[Unit] = {
    for {
  ...
  private def typeFromCache(name: String, allInfo: AllTypeInfo): TID = {
