...
trait FuncProps { this: FuncMixin =>
  ...
  case class TTestProp[TPE <: TType](
    ...
    tpe: TPE,
    tid: TID = TID(""),
    extras: Seq[PropExtras] = Seq.empty
  ) extends TProp[TPE] {
   ...
   * yet.
   */
  class TSystemProp[VT, RT, TPE <: TType](
    realProp: Property[VT, RT],
    val tpe: TPE,
    val extras: Seq[PropExtras] = Seq.empty
  ) extends TProp[TPE] {
    def tid = realProp.id
  ...
  object MembersRole extends TInstance("Members", querki.security.MOIDs.MembersTagOID)
