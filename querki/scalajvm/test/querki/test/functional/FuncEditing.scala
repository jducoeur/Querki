...
trait FuncEditing { this: FuncMixin =>
  ...
  def editorId(
    thing: TThing[_],
    prop: TProp[_]
  ): String = s"v-${prop.oidStr}-${thing.oidStr}"
  ...
  def designAModel(
    ...
    val stateWithThing =
      state.updateSpace(space => space.copy(things = space.things + (thingWithTID.tid -> thingWithTID)))
   ...
   */
  def addExistingProperty[TPE <: TType](
    prop: TProp[TPE]
  )(
    thing: TThing[_],
    state: State
  ...
  def createProperty[TPE <: TType](
    ...
    val propEditor = instanceSection.find(xpath("./li[last()]")).getOrElse(
      fail("Couldn't find PropValueEditor for newly-created Property!")
    )
  ...
  def createAnyThing(
    ...
    val editor =
      find(className("_instanceEditor")).getOrElse(fail("Couldn't find instance editor for newly-created Thing!"))
