...
The server **will refuse to start without `QUERKI_ENV` set.** `QuerkiEnv` reads it and the matching sub-config block is merged on top of the base config (see `QuerkiApplicationLoader.configurationWithEnv`). Values are (at least): `dev`, `prod`, `scenario`, `test`, `testnode`, `local`. Test infrastructure sets `scenario`; a local Docker run uses `local`.
...
`Running Querki Locally.md` at the repo root is Justin's authoritative walk-through for bringing up a dev install. It is the source of truth when debugging local-boot problems — consult it first. Key moving pieces it documents:

- **MySQL 5.7** (not 8.0): two DBs, `querkisystem` and `querkiuser`, owned by a `querki` account. First-time setup applies `querki/scalajvm/conf/evolutions/default/all.sql` (which has `TODO` lines that must be filled in, including the hashed admin password) to `querkisystem`.
- **Cassandra** via Docker: `cd querki/local/cassandra && docker compose up -d` (this replaced an older `ccm`-based flow; sections tagged **OLD** in the doc are strike-throughs of that obsolete path).
- **AWS LocalStack** stands in for Secrets Manager. Secrets are loaded with `awslocal secretsmanager create-secret --name querki/app/secrets --secret-string "$(< localsecrets.conf)"`. LocalStack state is transient, so secrets must be reloaded after every LocalStack restart.
- **Admin password hashing**: in the sbt console, `import querki.security.EncryptionUtil` and call `calcHash(password, 20000)` (iteration count must match `application.conf`).
- **Build + run**: `sbt docker:publishLocal` to produce the image, then `docker run --rm -e QUERKI_ENV=local -e AWS_ACCESS_KEY_ID=… -e AWS_SECRET_ACCESS_KEY=… -p 9000:9000 querkiserver:<version>`. The AWS creds need to exist but their values are irrelevant (LocalStack ignores them). Server lands on `http://localhost:9000`.
